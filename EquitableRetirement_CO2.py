...
class EquitableRetirement:
    ...
    def __buildModel(self,alpha,beta,gamma,DiscRate,jobCoeff):
        ...
        def Z(model):
            ...
            # Jobs removed from the objective statement to reflect new formaulation from 
        model.Z = pe.Objective(rule=Z, doc='Minimize system costs, health damages, while maximizing jobs')
        
        # constraints
        
        # new jobs rule 9/27/2021
        ...
        def CO2ReductionRuleLower(model,y):
            return sum(model.CO2Emissions[c,y] for c in model.C) >= model.CO2Limits[y]*0.8 # need to implement a sensitivity which is parametric (National 0.8) (State by state 0.5 or 0)
        ...
        def JobsConserveRuleLower(model,c,y):
            return model.COALOMEF[c]*(model.HISTGEN[c]-model.coalGen[c,y])*jobCoeff <= sum(model.CONEF[r,y]*model.capInvest[r,c,y]for r in model.R) #model.REOMEF[r,y]*model.reCap[r,c,y]+ 
        ...
        ''' 
        def JobsConserveRuleUpper(model,c,y):
            return model.COALOMEF[c]*(model.HISTGEN[c]-model.coalGen[c,y])*1.30 >= sum(model.REOMEF[r,y]*model.reCap[r,c,y]for r in model.R) # + model.CONEF[r,y]*model.capInvest[r,c,y] 
        model.JobsConserveRuleUpper = pe.Constraint(model.C,model.Y, rule=JobsConserveRuleUpper,doc = 'Conserve jobs within a range for updated formulation 9/27/2021')
         '''
        ...
        def balanceGenRule(model,c,y):
            return sum(model.reGen[r,c,y] for r in model.R) >= model.HISTGEN[c]-model.coalGen[c,y] # changed to >= to reflect formulation and jobs constraint
        ...
        def reGenRule(model,r,c,y):
            return model.reGen[r,c,y] <= model.CF[r]*model.reCap[r,c,y]*8760 # changed to equality based on conversation to test: MV 08102021 # changed to <= to reflect formulation changes 9/27/2021
        ...
        def capInvestLimit(model,r,c,y):
            ...
            return model.capInvest[r,c,y] <= model.MAXCAP[r,c]*model.reInvest[r,c,y]- model.reCap[r,c,y-1] # adjusted to ensure multiple build years doesn't exceed the limits of the site MV 10/02/2021
